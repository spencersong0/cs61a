...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #EC
        if isinstance(func, MacroProcedure):
            coolio = func.apply_macro(expr, env)
            return scheme_eval(coolio, env)
        #EC
        operator = rest.map(lambda y: scheme_eval(y, env))
        return scheme_apply(func, operator, env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        forms = Pair(bindings.first.first, forms)
        check_formals(forms)
        bod = Pair(scheme_eval(bindings.first.second.first, env), bod)
        bindings = bindings.second
    return env.make_child_frame(forms, bod)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        target4 = expressions.first
        check_formals(target.second)
        target2 = expressions.second
        macro = MacroProcedure(target.second, target2, env)
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while isinstance(result, Thunk):
            ...
            if scheme_symbolp(first) and first in SPECIAL_FORMS:
                ...
                # check_procedure(func)
                operator = rest.map(lambda y: scheme_eval(y, env))
                result = scheme_apply(func, operator, env)"""
        return result
